...
void Print_Unmapped(FILE* Single_File,READ & R,FILE* Single_File2,READ & R2,int Read_Len)
{
	...
	R2.Tag_Copy[Read_Len]=R2.Quality[Read_Len]=0;
	if(DASH_DEL)
	{
	...
	flockfile(Single_File);flockfile(Single_File2);
	fprintf(Single_File,">%s|%d\n%s\n",R.Description+1,INSERTSIZE,R.Tag_Copy);
	fprintf(Single_File2,">%s|%d\n%s\n",R2.Description+1,INSERTSIZE,R2.Tag_Copy);
	funlockfile(Single_File);funlockfile(Single_File2);
}
